![mckabue's stats](https://github-readme-stats-ecru-zeta-50.vercel.app/api?username=mckabue&theme=default&show_icons=true&hide_border=true&count_private=true&) ![mckabue's top languages](https://github-readme-stats-ecru-zeta-50.vercel.app/api/top-langs/?username=mckabue&theme=default&show_icons=true&hide_border=true&layout=compact&langs_count=8)

# Kabui Charles

Founder & Engineer · Ex-Microsoft

[Resume](https://mckabue.github.io) | [LinkedIn](https://www.linkedin.com/in/mckabue/) | [Email](mailto:mckabue@gmail.com)

## About Me

I build software and the companies around it. I now work full-time on Oark, writing at ToKnow.ai and building Blober.io. Before that, I spent five years at Microsoft building AI-powered, agentic experiences in M365 Copilot. Earlier I founded Oark Library and grew it to 130,000 users with almost no marketing budget before archiving it in 2024. I hold an MSc in Data Science from Strathmore and write at [ToKnow.ai](https://toknow.ai/).

I build things I think should exist.

## 🚀 Projects

- [Blober.io](https://blober.io/): local-first workflow engine for cloud storage transfers. Credentials never leave your machine.
- [ToKnow.ai](https://toknow.ai/): data-driven research on AI, security, and finance
- [@mckabue/react-validate](https://www.npmjs.com/package/@mckabue/react-validate): type-safe React form validation hook, zero dependencies
- [@mckabue/react-use-async](https://www.npmjs.com/package/@mckabue/react-use-async): React hook for async state with loading, error, and merge
- [@mckabue/no-same-type-params](https://www.npmjs.com/package/@mckabue/no-same-type-params): ESLint rule against same-type adjacent parameters
- [Oark Library](https://web.archive.org/web/20240917192838/https://oarklibrary.com/): education platform, 130,000+ users (archived 2024)

## 💼 Experience

- **Founder** at Oark (Jul 2026 - Present): ToKnow.ai, Blober.io
- **Software Engineer** at Microsoft (Apr 2021 - Jul 2026): M365 Copilot, Researcher, Copilot Chat
- **Android Engineer** at Hava Cab (Oct 2020 - Apr 2021)
- **Full-stack Engineer** at Maramoja Transport (May 2018 - Nov 2020)
- **Full-stack Engineer** at Pageone (Mar 2016 - Apr 2018)

## 🛠 Skills

- Languages: TypeScript, C#, Python, Java, Kotlin
- Frontend: React, Vue, HTML5, CSS3
- Backend: ASP.NET, Node.js, Flask
- Data: SQL, NoSQL, KustoQL, ElasticSearch
- Cloud: Azure, AWS
- AI/ML: LLM integration, agentic systems, M365 Copilot

## 🎓 Education

- MSc Data Science and Analytics, Strathmore University
- BSc Software Engineering, Kisii University

---

📄 [mckabue.github.io](https://mckabue.github.io) · ✍️ [ToKnow.ai](https://toknow.ai/) · 🏢 [oarklimited.com](https://oarklimited.com/)
